...
def restore_checkpoint(ckpt_dir, state, device, skip_sigma=False):
  ...
  return state
def restore_checkpoint_wcnn(ckpt_dir, state, device, skip_sigma=False):
  loaded_state = torch.load(ckpt_dir, map_location=device)
  ...
  loaded_model_state = {k.replace('module.',''):v for k,v in loaded_model_state.items()}
  #loaded_model_state = {k: v for k, v in loaded_model_state.items() if k in model_dict}
  state['model'].load_state_dict(loaded_model_state, strict=True)
  state['ema'].load_state_dict(loaded_state['ema'])
  state['step'] = loaded_state['step']
  print(f'loaded checkpoint dir from {ckpt_dir}')   
  return state
